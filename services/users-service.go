...
func (s *userService) GetUser(userId int64)  (*users.User, *rest_errors.RestError)  {
	result := &users.User{
		ID: userId,
	}
...
func  (s *userService) CreateUser(user users.User) (*users.User, *rest_errors.RestError)  {
...
func  (s *userService) UpdateUser(isPartial bool, user users.User) (*users.User, *rest_errors.RestError)  {
...
func  (s *userService) DeleteUser(userId int64) *rest_errors.RestError {
...
func  (s *userService) Search(status string) (users.Users, *rest_errors.RestError)  {
	dao := &users.User{}
	return dao.FindByStatus(status)
}

func  (s *userService) LoginUser(request users.LoginRequest)  (*users.User, *rest_errors.RestError)  {
	user := &users.User{
		Email: request.Email,
